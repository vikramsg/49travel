...
| metric filters | one bound per metric, a blank bound asking nothing of that metric; a usable set applies on submit and redraws without a page reload; an unusable bound — not a whole number, or off that metric's scale — is reported beside its own input and leaves the map on the last usable filters; the number of active filters is visible; Clear returns every metric to blank; an empty result names the filters as the cause rather than only suggesting a wider range |
