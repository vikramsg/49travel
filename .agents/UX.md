...
| metric filters | five bounds behind an "Advanced filters" button, closed on first load and showing how many are on; each bounds one metric, a blank bound asking nothing of that metric; a usable set applies on submit and redraws without a page reload; an unusable bound — not a whole number, or off that metric's scale — is reported beside its own input and leaves the map on the last usable filters; the applied bounds survive closing the button; Clear blanks every metric; the map states the active filters, so the applied set is readable without opening it |
