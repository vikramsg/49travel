...
- `python/src/trains/` — the map's batch builds: `city.py` (GeoNames cities and their stations), `links.py` (each city's Wikipedia and Wikivoyage article), `metrics.py` (one row of facts per city), `geo.py` (distance and radius indexing), `motis.py` and `travel_time.py` (the reachability measurement). `metrics.py` writes `city_metric.parquet`, where every fact is its own column — Wikipedia sitelinks, whether a Wikivoyage article exists, nearby World Heritage Sites, mapped tourism features, the Deutsche Bahn station category — and nothing is combined into a score.
...
- Nothing under `python/` needs network access for `make city_json`. `make city_links` and `make city_metrics` do: they read two wikis, Wikidata and the OpenStreetMap extracts. What they produce is committed, so the app never needs it. The archived scrapers needed network as well.
...
- `GET /api/reachable?origin=<city_id>&minHours=<n>&maxHours=<n>` returns `{measuredOn, origin: {cityId, name, latitude, longitude}, destinations: [{cityId, name, latitude, longitude, minutes, wikipediaUrl, wikivoyageUrl}]}`. `cityId` values are GeoNames ids as strings.
- A destination's `wikipediaUrl` and `wikivoyageUrl` are the English articles the pipeline resolved for it, or `null` where it resolved none; the popup links whichever are present and stays silent otherwise. `python/data/trains/city_link.parquet` holds them and `python/src/trains/links.py` builds it, offline-safe: nothing is resolved at request time.
- `city_metric.parquet` holds one row of facts per city, each fact its own column: `wikipedia_sitelinks`, `wikivoyage_article`, `unesco_sites`, `tourism_pois`, `db_station_category`. `python/src/trains/metrics.py` builds it from Wikidata and OpenStreetMap, both cached, so nothing is counted at request time either.
- Five optional filters narrow the answer, one per metric and each independent: `minWikipediaSitelinks`, `minWikivoyageArticles`, `minUnescoSites`, `minTourismPois` and `maxDbStationCategory`. Each is absent by default. A bound outside its range is a `400` like the band is, and a bound no destination satisfies is an empty answer rather than an error. `lib/city-metric-filters.ts` holds the bounds and the rule that a blank bound asks nothing, so the route, the SQL and the map's form cannot disagree.
...
- `/map` opens narrowed to a popular subset (`DEFAULT_METRIC_FILTERS` in `lib/city-metric-filters.ts`) rather than to every stop the range reaches: the origin and the travel-time band sit in a left sidebar, the five metric filters fold away behind a button, the applied ones are stated above the map, and one click returns the unfiltered set. The API's own default stays unfiltered, so a caller asking for a band still gets the band.
