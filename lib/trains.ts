 ...
 *
 * `minPopulation` and `maxPopulation` are the same kind of comparison against
 * `city.parquet`, so a size range narrows what is reachable without changing what
 * reachable means. A `maxPopulation` of null is the open top.
 */
  ...
  const reader = await connection.runAndReadAll(
    `SELECT t.city_id AS city_id, c.name AS name, c.latitude AS latitude,
     ...
     WHERE t.origin_city_id = ? AND t.city_id <> t.origin_city_id
       ...
       AND c.population >= ? AND c.population <= ?
    ...
    [
      ...
      maxMinutes,
      minPopulation,
      // The open top is written as the largest safe integer rather than left out
      // of the statement, so there is one parameterised query and no branch that
      // can drift from it. No population comes near this number.
